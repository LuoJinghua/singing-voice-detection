...
def get_random_wav(filenames, sec, sr=ModelConfig.SR):
    src1_src2 = map(
        lambda f: _sample_range(_pad_wav(librosa.load(f, sr=sr, mono=False)[0], sr, sec, pad_mode='wrap'), sr, sec),
        filenames)
...
def to_db(spec):
    ...
    return np.array(map(lambda s: librosa.power_to_db(s ** 2), spec))
...
def to_wav_mag_only(mag, init_phase, len_frame=ModelConfig.L_FRAME, len_hop=ModelConfig.L_HOP, num_iters=50):
    return np.array(map(lambda (m, p): griffin_lim(m, len_frame, len_hop, num_iters=num_iters, phase_angle=p),
                        zip(mag, init_phase)))
